...
const HdrIcon=({id,size=20})=>{
  ...
  if(id==="help")return(<svg width={size} height={size} viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" {...p}/><path d="M9.6 9.2a2.5 2.5 0 1 1 3.3 2.6c-.7.3-.9.8-.9 1.6" {...p}/><line x1="12" y1="16.6" x2="12" y2="16.7" {...p} strokeWidth={2.4}/></svg>);
...
export default function App(){
  ...
  return(
    <div style={{"--wpBlue":T.blue,"--wpBlueL":T.blueL,"--wpBlueDark":T.blueDark,height:"100dvh",width:"100%",overflow:"hidden",display:"flex",flexDirection:"column",background:T.bg,color:T.navy,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div style={{background:T.card,borderBottom:`1px solid ${T.border}`,padding:isMobile?"6px 10px":"8px 20px",display:"flex",alignItems:"center",minHeight:isMobile?52:64,gap:isMobile?6:8,flexWrap:"nowrap",flexShrink:0,width:"100%",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:isMobile?4:8,alignItems:"center",flexShrink:0}}>
          ...
          <button onClick={()=>{window.location.href=`mailto:wenceslyne@elitelvlservices.com?subject=${encodeURIComponent("Westcoast Poke Support Request")}&body=${encodeURIComponent(`Question about: \n\n\n—\nFrom: ${session.user?.email||"unknown user"}`)}`;}} className="wpHovG" {...tipBelow("Email support")} aria-label="Help" style={{background:"none",border:`1px solid ${T.border}`,borderRadius:20,color:T.muted,padding:isMobile?"5px 8px":"6px 14px",fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:6}}><HdrIcon id="help" size={isMobile?17:18}/>{!isMobile&&<span>Help</span>}</button>
      ...
      <div style={{display:"grid",gridTemplateColumns:isDesktop&&loc==="all"?"repeat(2,minmax(0,1fr))":"minmax(0,1fr)",gap:isMobile?12:16,marginBottom:isMobile?12:16,alignItems:"stretch"}}>
        ...
        <div className="wpLift" style={{...card,border:`1.5px solid ${T.blue}66`,height:"100%"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2,flexWrap:"wrap"}}>
            ...
            <button onClick={onRefreshLive} disabled={liveBusy} className="wpHovS" style={{marginLeft:"auto",background:liveBusy?T.bg:T.blue,color:liveBusy?T.muted:"#fff",border:"none",borderRadius:16,padding:"6px 14px",fontSize:12,fontWeight:700,cursor:liveBusy?"wait":"pointer"}}>{liveBusy?"Refreshing…":"⟳ Refresh live prices"}</button>
          </div>
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20,flexWrap:"wrap",gap:12}}>
        ...
        {aiInsights&&(
          <div style={{textAlign:"right"}}>
            <button onClick={generateInsights} disabled={loadingInsights} className="wpHovS" style={{position:"relative",background:T.blue,color:"#fff",border:"none",borderRadius:20,padding:"10px 20px",fontSize:14,fontWeight:700,cursor:loadingInsights?"not-allowed":"pointer",opacity:loadingInsights?0.7:1}}>
              ✦ Regenerate
